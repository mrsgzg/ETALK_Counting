...
from Sequence_Pooling_Classifier import create_sequence_pooling_model  # type: ignore
from DataLoader_sequence_pooling import get_sequence_pooling_data_loaders  # type: ignore
...
class SequencePoolingTrainer:
    """Trainer for sequence pooling classification using wandb logging."""
...
def build_model(use_pretrain: bool, pooling_strategy: str, num_classes: int, device: torch.device):
...
def run_training_once(exp_name: str,
                      ...
                      seed: int = 42,
                      project: str = 'sequence-pooling-counting',
    ...
    # model
    model = build_model(use_pretrain, pooling_strategy, num_classes=11, device=device)

    # trainer
    trainer = SequencePoolingTrainer(
        model=model,
